...
CREATE TABLE `farm_enterprise` (
    ...
    `name`        VARCHAR(100) NOT NULL                 COMMENT '浼佷笟鍚嶇О',
    `address`     VARCHAR(200) DEFAULT ''               COMMENT '浼佷笟鍦板潃',
    `lng`         DECIMAL(10,6) DEFAULT NULL            COMMENT '缁忓害',
    `lat`         DECIMAL(10,6) DEFAULT NULL            COMMENT '绾害',
    `icon`        VARCHAR(200) DEFAULT ''               COMMENT '浼佷笟鍥炬爣',
    `contact_name` VARCHAR(50) DEFAULT ''               COMMENT '鑱旂郴浜?,
    `contact_phone` VARCHAR(20) DEFAULT ''              COMMENT '鑱旂郴鐢佃瘽',
    `admin_id`    BIGINT       DEFAULT NULL             COMMENT '绠＄悊鍛樼敤鎴稩D锛堝叧鑱攕ys_user锛?,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='鍐滃満/浼佷笟琛?;
