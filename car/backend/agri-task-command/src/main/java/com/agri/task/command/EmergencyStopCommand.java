...
public class EmergencyStopCommand implements Command {
    ...
    public void execute(TaskCommand command) {
        log.warn("执行紧急停车命令: commandCode={}, vehicleId={}", command.getCommandCode(), command.getVehicleId());
        command.setStatus(Constants.CMD_SENT);
        command.setSentTime(LocalDateTime.now());
        commandMapper.updateById(command);

        String message = String.format(
                "{\"commandCode\":\"%s\",\"vehicleId\":%d,\"commandType\":\"EMERGENCY_STOP\",\"priority\":\"CRITICAL\",\"timestamp\":%d}",
    ...
    public boolean validate(TaskCommand command) {
        return command.getVehicleId() != null;
    }
}
